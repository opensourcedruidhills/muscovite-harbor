#pragma once
// SPDX-License-Identifier: AGPL-3.0-or-later OR LicenseRef-Muscovite-Commercial
// Copyright (c) 2025-2026 Johannes Lochmann
// GENERATED FILE — DO NOT EDIT

#include <string>
#include <utility>

namespace harbour_control {

/// Hazmat permit number (e.g. HP-2026-000123)
/// Strong type wrapper — prevents accidental mixing of types.
struct PermitNumber {
    using ValueType = std::string;

    ValueType value{};

    explicit PermitNumber() = default;
    explicit PermitNumber(ValueType v) : value{std::move(v)} {}

    [[nodiscard]] auto get() const noexcept -> const ValueType& { return value; }

    auto operator==(const PermitNumber&) const -> bool = default;
    auto operator<=>(const PermitNumber&) const = default;
};

} // namespace harbour_control
