#pragma once
// SPDX-License-Identifier: AGPL-3.0-or-later OR LicenseRef-Muscovite-Commercial
// Copyright (c) 2025-2026 Johannes Lochmann
// GENERATED FILE — DO NOT EDIT

#include <string>
#include <utility>

namespace harbour_control {

/// Safety zone code (e.g. SZ-NQ01)
/// Strong type wrapper — prevents accidental mixing of types.
struct ZoneCode {
    using ValueType = std::string;

    ValueType value{};

    explicit ZoneCode() = default;
    explicit ZoneCode(ValueType v) : value{std::move(v)} {}

    [[nodiscard]] auto get() const noexcept -> const ValueType& { return value; }

    auto operator==(const ZoneCode&) const -> bool = default;
    auto operator<=>(const ZoneCode&) const = default;
};

} // namespace harbour_control
