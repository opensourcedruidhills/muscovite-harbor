...
public:
    ...
    [[nodiscard]] auto build_select_by_id_query() const -> std::string {
        return "SELECT id, id, voyage_id, created_at, status FROM cargo_operations.load_plan WHERE id = $1";
    }

    [[nodiscard]] auto build_insert_query() const -> std::string {
        return "INSERT INTO cargo_operations.load_plan (id, id, voyage_id, created_at, status) VALUES ($1, $2, $3, $4, $5) RETURNING id";
    }

    [[nodiscard]] auto build_update_query() const -> std::string {
        return "UPDATE cargo_operations.load_plan SET id = $2, voyage_id = $3, created_at = $4, status = $5 WHERE id = $1";
    }

    [[nodiscard]] auto build_delete_query() const -> std::string {
        return "DELETE FROM cargo_operations.load_plan WHERE id = $1";
    }
    ...
    auto save(const LoadPlan& entity) -> void {
        auto tx = conn_.begin();
        tx.exec_params(build_insert_query(), entity.id, entity.id, entity.voyage_id, entity.created_at, entity.status);
        tx.commit();
    }

    auto update(const LoadPlan& entity) -> void {
        auto tx = conn_.begin();
        auto result = tx.exec_params(build_update_query(), entity.id, entity.id, entity.voyage_id, entity.created_at, entity.status);
