// SPDX-License-Identifier: AGPL-3.0-or-later OR LicenseRef-Muscovite-Commercial
// Copyright (c) 2025-2026 Johannes Lochmann
//
// This software is dual-licensed:
//  - GNU Affero General Public License v3 (AGPL-3.0), see LICENSE-AGPL
//  - Commercial license, contact muscovite-licensing@johanneslochmann.net
//
// Generated by Muscovite - DO NOT EDIT
// Any changes will be overwritten on regeneration.
// This file was generated by muscomp. DO NOT EDIT.

#pragma once

#include <QCheckBox>
#include <QDialog>
#include <QDialogButtonBox>
#include <QFormLayout>
#include <QLineEdit>
#include <QVBoxLayout>
#include <memory>
#include <QVariantMap>
#include "common/entity_edit_dialog_base.hpp"

namespace muscovite_harbor::widgets {

class ReeferUnitEditDialog : public EntityEditDialogBase {
    Q_OBJECT
public:
    explicit ReeferUnitEditDialog(QWidget* parent = nullptr);

    /// Create and show dialog safely. Ownership transfers to Qt on success.
    static void showSafe(QWidget* parent) {
        auto dlg = std::unique_ptr<ReeferUnitEditDialog>{new ReeferUnitEditDialog{parent}};
        dlg->setAttribute(Qt::WA_DeleteOnClose);
        dlg.release()->show();
    }

    [[nodiscard]] auto id() const -> QString;
    [[nodiscard]] auto containerId() const -> QString;
    [[nodiscard]] auto targetTempC() const -> QString;
    [[nodiscard]] auto currentTempC() const -> QString;
    [[nodiscard]] auto isPowered() const -> bool;

    void setId(const QString& value);
    void setContainerId(const QString& value);
    void setTargetTempC(const QString& value);
    void setCurrentTempC(const QString& value);
    void setIsPowered(const bool& value);

Q_SIGNALS:
    void saveRequested(const QVariantMap& payload);
    void entitySaved();
    void deleteRequested();

private Q_SLOTS:
    void onAccepted();

private:
    void setupUi();
    [[nodiscard]] auto buildPayload() const -> QVariantMap;

    QLineEdit* id_ = nullptr;
    QLineEdit* container_id_ = nullptr;
    QLineEdit* target_temp_c_ = nullptr;
    QLineEdit* current_temp_c_ = nullptr;
    QCheckBox* is_powered_ = nullptr;
    QDialogButtonBox* button_box_ = nullptr;
    QFormLayout* form_ = nullptr;
    ValidationDisplayWidget* validation_display_ = nullptr;
};

} // namespace muscovite_harbor::widgets

